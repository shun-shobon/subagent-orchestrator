...
3. 参照文書を確認する。
   ...
   - `orchestration/tasks/<task-id>/task-execution-output.md`
...
3. 実施内容、検証結果、残課題を `orchestration/tasks/<task-id>/task-execution-output.md` に記録する。
4. 必要な場合はブロッカーを明記し、プロジェクト管理エージェントへエスカレーションする。

実装担当の最小出力:

- 割り当て範囲に限定したコード変更。
- DoDに対応するテスト証跡。
- `orchestration/tasks/<task-id>/task-execution-output.md` の更新。
...
- タスク実行エージェント契約: `references/task-execution-agent-contract.md`
